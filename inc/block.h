...
class BLOCK {
 public:
  uint8_t valid, prefetch, dirty, used, l2_prefetch_hit;

  int delta, depth, signature, confidence, prefetch_offset;

  uint64_t address, full_addr, tag, data, cpu, instr_id, event_cycle;
  ...
  BLOCK() {
    ...
    dirty           = 0;
    used            = 0;
    l2_prefetch_hit = 0; // added for removing LLC useless by L2 useful
...
class PACKET_QUEUE {
 public:
  ...
  uint32_t cpu, head, tail, occupancy, num_returned, next_fill_index, next_schedule_index,
      next_process_index;

  uint64_t next_fill_cycle, next_schedule_cycle, next_process_cycle, ACCESS, FORWARD, MERGED,
      TO_CACHE, ROW_BUFFER_HIT, ROW_BUFFER_MISS, FULL;
...
class CORE_BUFFER {
 public:
  const string   NAME;
  const uint32_t SIZE;
  uint32_t cpu, head, tail, occupancy, last_read, last_fetch, last_scheduled, inorder_fetch[2],
      next_fetch[2], next_schedule;
  uint64_t event_cycle, fetch_event_cycle, schedule_event_cycle, execute_event_cycle,
      lsq_event_cycle, retire_event_cycle;
