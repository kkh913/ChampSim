...
uint8_t warmup_complete[NUM_CPUS], simulation_complete[NUM_CPUS], all_warmup_complete = 0, all_simulation_complete = 0, MAX_INSTR_DESTINATIONS = NUM_INSTR_DESTINATIONS, knob_cloudsuite = 0,
...
void print_roi_stats(uint32_t cpu, CACHE *cache) {
  ...
  cout << "  USEFUL: " << setw(10) << cache->pf_useful << "  USELESS: " << setw(10) << cache->pf_useless << "  LATENESS: " << setw(10) << cache->pf_late << "  USELESS_BY_PF_HIT: " << setw(10)
       << cache->pf_useless_by_pf_hit << endl;
}
...
void print_dram_stats() {
  ...
  for (uint32_t i = 0; i < DRAM_CHANNELS; i++) total_congested_cycle += uncore.DRAM.dbus_cycle_congested[i];
    ...
    if (allocated_pages >= DRAM_PAGES) {  // not enough memory
...
#ifdef SANITY_CHECK
      if (ppage_check == inverse_table.end()) assert(0);
#endif
      ...
      while (1) {                                        // try to find an empty physical page number
        ...
        if (ppage_check != inverse_table.end()) {        // random_ppage is not available
          ...
          if (num_adjacent_page > 0) fragmented = 1;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < argc; i++) {
    if (found_traces) {
      ...
      char *full_name = ooo_cpu[count_traces].trace_string, *last_dot = strrchr(ooo_cpu[count_traces].trace_string, '.');
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
      ...
      if (stall_cycle[i] <= current_core_cycle[i]) {
        ...
        if (ooo_cpu[i].ROB.occupancy < ooo_cpu[i].ROB.SIZE) {
          // handle branch
          if (ooo_cpu[i].fetch_stall == 0) ooo_cpu[i].handle_branch();
        }
        ...
        if ((ooo_cpu[i].ROB.entry[schedule_index].scheduled == 0) && (ooo_cpu[i].ROB.entry[schedule_index].event_cycle <= current_core_cycle[i])) ooo_cpu[i].schedule_instruction();
        ...
        // retire
        if ((ooo_cpu[i].ROB.entry[ooo_cpu[i].ROB.head].executed == COMPLETED) && (ooo_cpu[i].ROB.entry[ooo_cpu[i].ROB.head].event_cycle <= current_core_cycle[i])) ooo_cpu[i].retire_rob();
      }
      ...
      // check for deadlock
      if (ooo_cpu[i].ROB.entry[ooo_cpu[i].ROB.head].ip && (ooo_cpu[i].ROB.entry[ooo_cpu[i].ROB.head].event_cycle + DEADLOCK_CYCLE) <= current_core_cycle[i]) print_deadlock(i);
      ...
      */
      ...
      if (all_simulation_complete == NUM_CPUS) run_simulation = 0;
    }
