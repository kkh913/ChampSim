...
void CACHE::handle_fill() {
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if (do_fill) {
      // update prefetcher
      if (cache_type == IS_L1D) l1d_prefetcher_cache_fill(MSHR.entry[mshr_index].full_addr, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, block[set][way].full_addr);
      ...
      if (cache_type == IS_L1D) {
        if (MSHR.entry[mshr_index].type == RFO) block[set][way].dirty = 1;
      }
      ...
      if (cache_type == IS_ITLB) {
        ...
        if (PROCESSED.occupancy < PROCESSED.SIZE) PROCESSED.add_queue(&MSHR.entry[mshr_index]);
      ...
      if (cache_type == IS_L1D) {  // RFO miss
        ...
        if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) {  // this is a new miss
          ...
          if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) {  // not enough MSHR resource
            ...
            // update fill_level
            if (WQ.entry[index].fill_level < MSHR.entry[mshr_index].fill_level) MSHR.entry[mshr_index].fill_level = WQ.entry[index].fill_level;
            ...
            if (MSHR.entry[mshr_index].type == PREFETCH) {
              ...
              // in case request is already returned, we should keep event_cycle and retunred
              // variables
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0)) {
      ...
      if (way >= 0) {  // read hit
        ...
        if (cache_type == IS_ITLB) {
          ...
          if (PROCESSED.occupancy < PROCESSED.SIZE) PROCESSED.add_queue(&RQ.entry[index]);
        } else if (cache_type == IS_DTLB) {
        ...
        if (RQ.entry[index].type == LOAD) {
          if (cache_type == IS_L1D)
            ...
            l2c_prefetcher_operate(block[set][way].full_addr, RQ.entry[index].ip, 1, block[set][way].prefetch);
        }
        ...
        if (block[set][way].prefetch) {
          ...
          if (lower_level && cache_type == IS_L2C) {
            lower_level->add_l2_prefetch_hit(block[set][way].address);  // added for removing LLC useless by L2 useful.
          }
        ...
        if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) {  // this is a new miss
          ...
          if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) {  // not enough MSHR resource
            ...
            // update fill_level
            if (RQ.entry[index].fill_level < MSHR.entry[mshr_index].fill_level) MSHR.entry[mshr_index].fill_level = RQ.entry[index].fill_level;
            ...
            if (MSHR.entry[mshr_index].type == PREFETCH) {
              ...
              pf_late++;  // added. Late prefetch when a demand miss occurred before the prefetch response is fetched to the cache.
            }
        ...
        if (miss_handled) {
          // update prefetcher on load instruction
          if (RQ.entry[index].type == LOAD) {
            if (cache_type == IS_L1D) l1d_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type);
            if (cache_type == IS_L2C) l2c_prefetcher_operate(RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type);
          }
      ...
      if (way >= 0) {  // prefetch hit
        ...
        if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) {  // this is a new miss
          ...
          if (lower_level) {
            if (cache_type == IS_LLC) {
              if (lower_level->get_occupancy(1, PQ.entry[index].address) == lower_level->get_size(1, PQ.entry[index].address))
                ...
                if (PQ.entry[index].fill_level <= fill_level) add_mshr(&PQ.entry[index]);
          ...
          if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) {  // not enough MSHR resource
            ...
            // update fill_level
            if (PQ.entry[index].fill_level < MSHR.entry[mshr_index].fill_level) MSHR.entry[mshr_index].fill_level = PQ.entry[index].fill_level;
...
void CACHE::operate() {
  ...
  if (PQ.occupancy && (RQ.occupancy == 0)) handle_prefetch();
}

uint32_t CACHE::get_set(uint64_t address) { return (uint32_t)(address & ((1 << lg2(NUM_SET)) - 1)); }

uint32_t CACHE::get_way(uint64_t address, uint32_t set) {
  for (uint32_t way = 0; way < NUM_WAY; way++) {
    if (block[set][way].valid && (block[set][way].tag == address)) return way;
  }
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet) {
  ...
  if (block[set][way].prefetch && (block[set][way].used == 0)) {
    if (cache_type == IS_LLC) {
      if (block[set][way].l2_prefetch_hit) {
        pf_useless_by_pf_hit++;  // count up LLC useless by L2 useful.
      } else {
        pf_useless++;
      }
    } else {
  ...
  block[set][way].l2_prefetch_hit = 0;
...
int CACHE::add_rq(PACKET *packet) {
  ...
  if (wq_index != -1) {
    ...
    if ((cache_type == IS_L1D) && (packet->type != PREFETCH)) {
      if (PROCESSED.occupancy < PROCESSED.SIZE) PROCESSED.add_queue(packet);
...
int CACHE::add_pq(PACKET *packet) {
  ...
  if (index != -1) {
    if (packet->fill_level < PQ.entry[index].fill_level) PQ.entry[index].fill_level = packet->fill_level;
...
// mark L2 prefetch hit in an LLC prefetched block which has a same address.
// this function is only statistics. It is not included in the actual implementation.
void CACHE::add_l2_prefetch_hit(uint64_t address) {
  uint32_t set = get_set(address);
  uint32_t way = get_way(address, set);

  if (way < NUM_WAY && block[set][way].prefetch) {
    block[set][way].l2_prefetch_hit = 1;
  }
}
